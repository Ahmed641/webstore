...
(function(a){a.waitUntil=function(h,i,d){var e=d||200;
var f=i||20;
var b=$.Deferred();
var c=0;
var g=setInterval(function(){if(c<f){if(h()){clearInterval(g);
b.resolve()
}else{c+=1
...
(function(a){a.Device=function(){};
a.Device.prototype.ua=function(){return navigator.userAgent
};
a.Device.prototype.uaContains=function(b,c){if(!c){c=this.ua()
}return c.toLowerCase().indexOf(b)>-1
};
a.Device.prototype.iPhone=function(){return this.uaContains("iphone")
};
a.Device.prototype.iPod=function(){return this.uaContains("ipod")
};
a.Device.prototype.iPad=function(){return this.uaContains("ipad")
};
a.Device.prototype.ios=function(){return this.iPhone()||this.iPod()||this.iPad()
};
a.Device.prototype.android=function(){return this.uaContains("android")
};
a.Device.prototype.msie=function(){return this.uaContains("msie")
};
a.Device.prototype.trident=function(){return this.uaContains("trident")
};
a.Device.prototype.edge=function(){return this.uaContains("edge")
};
a.Device.prototype.ms=function(){return this.msie()||this.trident()||this.edge()
};
a.Device.prototype.mac=function(){if(navigator.platform.toUpperCase().indexOf("MAC")!==-1){return true
}};
a.Device.prototype.win=function(){if(navigator.platform.toUpperCase().indexOf("WIN")!==-1){return true
}};
a.Device.prototype.linux=function(){if(navigator.platform.toUpperCase().indexOf("LINUX")!==-1){return true
}};
a.Device.prototype.app=function(){return this.uaContains("targetapp_")||window.isMobileApp
};
a.device=new a.Device()
})(window.hm);
(function(c){c.Tracking={debug:false,logger:function(){this.debug&&typeof console!=="undefined"&&console.log.apply(console,arguments)
},link:function(d){window.sessionStorage.setItem("utagClickData",JSON.stringify(d));
var e=c.device.app()?"event":"link";
this.tracker(e,d)
},view:function(d){this.tracker("view",d)
},appPageView:function(d){if(c.device.app()){this.tracker("appPageView",d)
}},tracker:function(h,f){var e=this;
var g=(c.device.iPhone()&&c.options.forceUtagForIOS)||(c.device.android()&&c.options.forceUtagForAndroid)||(c.device.iPad()&&c.options.forceUtagForIOS);
if(c.device.app()&&!g){var d={type:h,payload:f,purchase:$(".order-confirmation").length?1:0};
if(c.device.iPhone()){c.util.waitUntil(function(){return(window.webkit&&window.webkit.messageHandlers&&window.webkit.messageHandlers.NativeTracker&&window.webkit.messageHandlers.NativeTracker.postMessage)
}).done(function(){window.webkit.messageHandlers.NativeTracker.postMessage(d)
}).fail(function(){if(h==="appPageView"){h="view"
}window.utag&&window.utag[h]&&window.utag[h](f)
})
}else{if(c.device.android()){c.waitUntil(function(){return window.NativeTracker&&window.NativeTracker.postMessage
}).done(function(){window.NativeTracker.postMessage(JSON.stringify(d))
}).fail(function(){if(h==="appPageView"){h="view"
}window.utag&&window.utag[h]&&window.utag[h](f)
})
}else{if(c.device.iPad()){c.util.waitUntil(function(){return(window.webkit&&window.webkit.messageHandlers&&window.webkit.messageHandlers.NativeTracker&&window.webkit.messageHandlers.NativeTracker.postMessage)
}).done(function(){window.webkit.messageHandlers.NativeTracker.postMessage(d)
}).fail(function(){if(h==="appPageView"){h="view"
}window.utag&&window.utag[h]&&window.utag[h](f)
})
}else{e.logger("Unsupported device for analytics bridge for mobile")
}}}}else{c.waitUntil(function(){var i=window.utag&&typeof window.utag[h]==="function";
e.logger("**Tracking** is utag ready?",i);
return i
}).done(function(){window.sessionStorage.setItem("utagClickData",null);
e.logger("**Tracking** window.utag.link",f);
window.utag[h](f)
})
}}};
try{var a=JSON.parse(window.sessionStorage.getItem("utagClickData"));
if(a){c.Tracking.link(a);
window.sessionStorage.setItem("utagClickData",null)
}}catch(b){console.error("Error parsing",b.message);
console.log(window)
}})(window.hm);
...
}}function utagTrackFormView(e,b,a,d,c){hm.Tracking.view({display_language:d,page_id:a,category_id:e,category_path:b,selected_market:c,touchpoint:getTouchpoint()})
...
}}function utagLinkBanner(){hm.Tracking.link({event_type:"PROMOTIONCLICK",promotion_id:["Campaign"],promotion_name:["Loyalty"],promotion_creative:["Teaser"],promotion_position:[utag_data.page_id]});
createUtagCookie("Registration : Join H&M Club","Registration",1)
}function utagLinkOrderConfirmation(){hm.Tracking.link({event_type:"PROMOTIONCLICK",promotion_id:["Campaign"],promotion_name:["Loyalty"],promotion_creative:["Teaser"],promotion_position:[utag_data.page_id]})
}function trackSignup(){hm.Tracking.link({event_type:"NEWSLETTER_SIGNUP",event_category:"Newsletter",event_id:"Signup",newsletter_signup:"1"})
}function trackJoinClub(b){console.log("trackJoinClub initiate");
if(typeof utag_data!=="undefined"){var a="Header";
if(utag_data.page_id=="CHECKOUT : LOGIN"&&!b){a="Checkout Login"
}if(utag_data.page_id=="CART"&&!b){a="Shopping bag"
}if(utag_data.page_id=="LOGIN"&&!b){a="Login"
}hm.Tracking.link({event_type:"CLUB_JOIN_INITIATE",event_category:"Club",event_id:"Join club initiated",event_label:a})
...
utagTrackFormView("LOGIN","LOGIN","Login : Header",localeInfo.locale,localeInfo.country)
}function trackLogout(){hm.Tracking.link({event_type:"LOGIN_LOGOUT",event_id:"Logout",event_category:"Login"})
}function trackSubmitJoin(a){if(hm.options.isLoyaltyMarket){trackJoinClub(a)
}}function utagHMNewsLetterSignupEvent(){hm.Tracking.link({event_type:"NEWSLETTER_SIGNUP",event_category:"Newsletter",event_id:"Signup",newsletter_signup:"1"})
}function utagTrackClubJoinSuccess(b){console.log("utagTrackClubJoinSuccess");
if(typeof utag_data!=="undefined"){var a="Header";
if(utag_data.page_id=="CHECKOUT : LOGIN"&&!b){a="Checkout Login"
}if(utag_data.page_id=="CART"&&!b){a="Shopping bag"
}if(utag_data.page_id=="LOGIN"&&!b){a="Login"
}hm.Tracking.link({event_type:"CLUB_JOIN_SUCCESS",event_id:"Join club succeeded",event_category:"Club",event_label:a})
}}function utagTrackClubJoinFailure(){console.log("utagTrackClubJoinFailure");
hm.Tracking.link({event_type:"CLUB_JOIN_ERROR",event_id:"Join club unsuccessful",event_category:"Club",event_label:"Club Join Error"})
}function utagClubAndFNSignupEvent(){hm.Tracking.link({event_type:"CLUB_SIGNUP",event_category:"Club",event_id:"Signup",club_signup:"1",newsletter_signup:"1"})
...
b.push(d.ranSiteID);
e.push(d.url);
hm.Tracking.link({aff_referrer:a,aff_network:f,aff_publisher:c,aff_site_id:b,aff_target_page:e})
}function utagRakutenRedirectLink(){hm.cookies=hm.cookies||new hm.Cookies();
var b;
var d="affiliateCookie";
b=hm.cookies.readCookie(d);
if((b!==null)&&(typeof(b)!=="undefined")){var a=JSON.parse(b);
if(typeof(a)=="string"){a=JSON.parse(a)
}if((!a.rakutenExecuted)){utagRakutenRedirectLinkEvent(a);
a.rakutenExecuted=true;
var c=a.cookieDuration/60;
hm.cookies.createCookie(d,JSON.stringify(a),null,c,true,".hm.com")
...
var e=$(this);
e.attr("id",h);
var d={id:h,elm:e,type:e.attr("data-activity-type")?e.attr("data-activity-type"):"",code:e.attr("data-activity-code")?e.attr("data-activity-code"):"",creative:e.attr("data-promotion-creative")?e.attr("data-promotion-creative"):"",segment:e.attr("data-promotion-segment")?e.attr("data-promotion-segment"):"",position:e.attr("data-promotion-position")?e.attr("data-promotion-position"):"",percentage:e.attr("data-tracking-percentage")?e.attr("data-tracking-percentage"):"",segment:e.attr("data-promotion-segment")?e.attr("data-promotion-segment"):"",top:e.offset().top,left:e.offset().left,width:e.width(),height:e.height(),tracked:false,viewTrackingEnabled:e.attr("data-newarrivals-first-article")?e.attr("data-newarrivals-first-article")==="true":true};
...
a.checkInterval=setInterval(function(){var u=document.documentElement,c=(window.pageXOffset||u.scrollLeft)-(u.clientLeft||0),o=(window.pageYOffset||u.scrollTop)-(u.clientTop||0),n=$(window).height(),b=$(window).width(),j,l=[],t=[],g=[],h=[],m=[];
var r=sessionStorage.getItem("debugTracking"),d=window.debugTracking||r;
if(d==true){if(r!=true){try{sessionStorage.setItem("debugTracking",d)
}catch(f){}}}for(var q=0;
q<a.trackedElementsInPage.length;
++q){var s=a.trackedElementsInPage[q];
if(s.viewTrackingEnabled){s.top=s.elm.offset().top;
s.left=s.elm.offset().left;
s.width=s.elm.width();
s.height=s.elm.height();
if(d){if(s.tracked){$("#"+s.id).css("border","3px solid red")
}else{$("#"+s.id).css("border","3px solid blue")
}}if((s.percentage!="undefined")&&(s.percentage!="")){var v=((s.top)+s.height*s.percentage);
if(s.height>0&&s.width>0&&o>=o&&v<=o+n&&s.left>=c&&(s.left+s.width)<=c+b){if(!s.tracked){s.tracked=true;
if(hm.eventBus){hm.eventBus.emit("trackable-event",{type:"pra_visualization",el:s.elm})
}}}}else{if(s.height>0&&s.width>0&&o>=o&&(s.top+s.height)<=o+n&&s.left>=c&&(s.left+s.width)<=c+b){if(!s.tracked){j=s.tracked=true;
l.push(s.type);
t.push(s.code);
g.push(s.creative);
h.push(s.position);
...
a.isViewClick=false
}var p={event_type:k,event_category:"Internal Promotions",event_id:"view",promotion_name:l.slice(-1),promotion_id:t.slice(-1),promotion_segment:m.slice(-1),promotion_creative:g.slice(-1),promotion_position:h.slice(-1)};
hm.Tracking.link(p);
if(a.debugTracking){console.log("viewtrack utag.link sent with payload:");
console.log(p)
}}},500)
})(promotionTracking);
$(function(){var b=[];
var a=Math.floor(Math.random()*10000000);
$("*[data-tracking-type]").each(function(){var e=$(this);
var c;
if(e.data("tracking-json-template")){c=$("#"+e.data("tracking-json-template")).text();
var f=e.data("tracking-params").split("|");
$.each(f,function(h){c=c.replace("{"+h+"}",this)
})
}else{c=e.data("tracking-params")
}var g;
try{g=JSON.parse(c.trim())
}catch(d){console.log("Error parsing json params for the tracking call");
return true
}if(e.data("tracking-type")=="link"){e.click(function(){hm.Tracking.link(g)
})
}})
});
...
if(jsonParams){if(type==priv.eventType.ADD_FAVORITE){jsonParams.event_type="ADD_TO_FAVOURITES";
...
console.log(jsonParams)
}}else{if(type==priv.eventType.PRA_STYLE_WITH_SWIPE||type==priv.eventType.PRA_STYLE_WITH_CLICK){if(priv.debugTracking){console.log("viewtrack utag.link will be sent with payload:");
console.log(jsonParams)
}hm.Tracking.link(jsonParams)
...
}function getProductAvailability(a){$.ajax({dataType:"json",method:location.hostname==="localhost"?"POST":"GET",url:hm.options.product.productAvailabilityServiceUrlResolved,data:"",processData:false,success:function(b){window.productAvailability=b
...
h.date=d.getTime();
h.counter=h.counter+1
}}}hm.cookies.createCookie("clubFullJoinCookie",JSON.stringify(h),null,null,true)
}}}return j
};
...
}}$(document).ready(function(){var g=$(window.frameElement);
if(g.length){g.parent().parent().prepend("<style type='text/css'>.setHeigthforModal {height: 100vh!important} iframe[name=livePreviewFrame] { position: relative!important;}</style>");
...
}if($(".js-custom-data").length){var a=JSON.parse(hm.cookies.readCookie("userCookie"));
if(typeof a!=="object"){a=JSON.parse(a)
}if(a&&a.hybris_uuid){var h=a.hybris_uuid,f=a.firstName||a.hybris_firstName,c=$(".js-custom-data").html();
if(h){var j=/\[\email\]/gmi;
c=c.replace(j,h)
}if(f){var i=/\[\username\]/gmi;
c=c.replace(i,f)
}$(".js-custom-data").html(c)
}}hm.cookies=hm.cookies||new hm.Cookies();
...
var n=JSON.parse(hm.cookies.readCookie("userCookie")),q=n.bpid,w=utag_data.customer_id,o=utag_data._ccity,m=utag_data._ccountry,x=utag_data.customer_email,r=utag_data._cstate,t=utag_data._czip,l=utag_data.customer_fashion_news,p=utag_data.customer_loyalty_level,s=utag_data.customer_has_children,u=utag_data.customer_status,k=utag_data.customer_club_member,v=utag_data.customer_club_level;
hm.Tracking.link({event_type:"REGISTRATION",event_id:"Login : Auto",event_category:"Login",customer_bpid:q,customer_id:w,customer_city:o,customer_country:m,customer_email:x,customer_state:r,customer_zip:t,customer_fashion_news:l,customer_loyalty_level:p,customer_has_children:s,customer_status:u,customer_club_member:k,customer_club_level:v})
}else{console.log("utagMainScript is undefined")
}},2000)
}if(location.pathname.indexOf("orderConfirmation")>-1&&window.orderConfirmationGuestEmail){var e=document.createElement("p");
e.innerHTML=window.orderConfirmationGuestEmail;
$formOrderConfirmation=$("form.order-confirmation");
if($formOrderConfirmation.length){$("form.order-confirmation .inputwrapper")[0].appendChild(e);
$("#signin-email").attr({type:"hidden"});
$("#signin-email").val(window.orderConfirmationGuestEmail);
$("#signin-email").change()
}}var b=function(k){var l=hm.Remodal.Utils.Helper.getModalId(k);
if(l==="modalSustainability"){hm.options.sustainabilityClicked=true
}};
hm.eventBus=hm.eventBus||new hm.EventEmitter();
hm.eventBus.addListener("remodal.afterShow",b);
function d(k){var l=hm.Remodal.Utils.Helper.getModalId(k);
if(l==="myid"){}}});
function trackVideoStart(b,c){var a;
if(c==true){a="VIDEO_AUTOSTART"
}else{a="VIDEO_START"
}hm.Tracking.link({event_type:a,event_id:"Started",event_category:"Video",event_label:b})
}function trackVideoPause(c,d,a){var b=getEventId(d,a);
hm.Tracking.link({event_type:"VIDEO_PAUSE",event_id:b,event_category:"Video",event_label:c})
}function trackVideoStop(c,d,a){var b=getEventId(d,a);
hm.Tracking.link({event_type:"VIDEO_STOP",event_id:b,event_category:"Video",event_label:c})
}function trackVideoCompleted(a){hm.Tracking.link({event_type:"VIDEO_COMPLETED",event_id:"100%",event_category:"Video",event_label:a})
...
$("#doiMiddlePageDestinationUrl").on("click",function(){var a=$(this).data("url");
setTimeout(function(){window.location.href=a
},500)
});
...
var e=$formButton.closest("*[data-remodal-id]").length>0;
trackSubmitJoin(e);
...
$.ajax({type:"POST",url:b,data:JSON.stringify(a),dataType:"json",contentType:"application/json; charset=utf-8",success:function(g){if(g.errors==null){if(g.hmNewsSubscription==true){utagHMNewsLetterSignupEvent()
}if(g.hmClubJoin==true){if(g.losServiceDown){utagTrackClubJoinFailure()
}else{utagTrackClubJoinSuccess(e)
}}if(g.doiMarket&&g.emailUnconfirmed&&g.hmNewsSubscription){console.log("we are in a doi market and the user registerd in checkout -- opening popup");
$("#doiMiddlePageDestinationUrl").attr("data-url",$("#contextPath").val()+g.redirect);
var f=g.email;
$("#doiMiddlePageGuestSignup").find("p#user-email").text(function(){return $(this).text().replace("{0}",f)
});
if(g.hmClubJoin){console.log("open popup DOI for club and setting OVERVIEW url for button: "+$("#contextPath").val()+g.redirect);
...
utagGuestCheckoutSignupView(g.email)
}else{window.location=$("#contextPath").val()+g.redirect
}}else{window.location=$("#contextPath").val()+g.redirect
}}else{utagTrackClubJoinFailure();
$.each(g.errors.registerForm,function(h,i){hm.formValidator.displayFieldStatus($form.find("input[name = "+h+"]"),{isValid:false,errorMessage:i})
})
}},failure:function(f){utagTrackClubJoinFailure()
},complete:function(){$formButton.attr("disabled",false)
}})
}else{var d=$formButton.attr("data-submit-redirect");
$.ajax({type:"POST",url:d,data:JSON.stringify(a),dataType:"json",contentType:"application/json; charset=utf-8",success:function(f){if(f.errors==null){if(f.hmNewsSubscription==true){utagHMNewsLetterSignupEvent()
}if(f.hmClubJoin==true){if(f.losServiceDown){utagTrackClubJoinFailure()
}else{utagTrackClubJoinSuccess(e)
}}window.location="/"+hm.options.product.locale+f.redirect
}else{utagTrackClubJoinFailure();
...
hm.util.scrollToElement($form.find("input[name = "+g+"]").attr("id"),-80)
})
}},failure:function(f){utagTrackClubJoinFailure()
},complete:function(){$formButton.attr("disabled",false)
}})
...
var b=hm.cookies.readCookie(hm.cookies.userCookieName);
var d=jQuery.parseJSON(b);
var a=window.location.pathname;
var e=a.split("/");
var c=e[e.length-1];
if(d&&d.customer_id!=null){if((!a.indexOf("/account")>-1)&&!(c.indexOf("account")>-1)){$.ajax({type:"GET",url:"/"+hm.options.product.locale+"/v1/user",contentType:"application/json; charset=utf-8",success:function(f){$("form.js-form-clubjoin div.select-date div").children("input[type = 'tel']").each(function(h){var g=$(this).attr("name");
$(this).val(f[g]);
if(f.cbpcustomer&&f[g]){$(this).attr("disabled","true");
$(this).parent().addClass("disabled")
}});
$("form.js-form-clubjoin div.select-date div").children("input[type = 'tel']").each(function(g){$(this).blur()
})
},failure:function(f){console.log("Error in v1/user ajax post")
}})
}}$("body").on("click",".js-submit-clubjoin",function(g){g.preventDefault();
hm.formValidator=new hm.FormValidator();
$formButton=$(this);
var j=$formButton.closest("*[data-remodal-id]").length>0;
if(hm.formValidator.validateForm($("form.form.js-form-clubjoin"),true)){var i="/"+hm.options.product.locale+"/hmclub/register-club";
var h=this;
h.setAttribute("disabled","disabled");
var f=$("form.form.js-form-clubjoin").serializeObject();
var k=Object.keys(f).map(function(l){return encodeURIComponent(l)+"="+encodeURIComponent(f[l])
}).join("&");
$.ajax({type:"POST",url:i,data:k,dataType:"json",contentType:"application/x-www-form-urlencoded",success:function(m){if(m.responseStatusCode==="ok"){console.log("200 OK ");
utagTrackClubJoinSuccess(j);
var l=window.location.hash;
hm.confirmExit.toggle(false);
window.location.href=window.location.href.replace(l,"")+"?upgraded=true"
}else{if(m.responseStatusCode==="ko"){console.log("200 KO")
}else{console.log("responseStatusCode not set")
}}},failure:function(l){console.log(" 500 Error")
},complete:function(){h.setAttribute("disabled",false);
...
var g=$(this).closest("form").find("input[type='password']");
var f=$(this).closest("*[data-remodal-id]").length>0;
if(!hm.formValidator.validateForm(c,true)){return
}a.setAttribute("disabled",true);
var e=new XMLHttpRequest();
e.onreadystatechange=function(){if(this.readyState==4&&this.status==200){var h=this.responseURL;
if(typeof utag_data!=="undefined"){var i="Header";
if(utag_data.page_id=="CHECKOUT : LOGIN"&&!f){i="Checkout Login"
}if(utag_data.page_id=="CART"&&!f){i="Shopping bag"
}if(utag_data.page_id=="LOGIN"&&!f){i="Login"
}hm.Tracking.link({event_type:"LOGIN_SUCCEEDED",event_id:"Login succeeded",event_category:"Login",event_label:i})
}if(h==undefined){window.location.reload()
}else{window.location.replace(h)
}}else{if(this.readyState==4&&this.status==401){hm.formValidator.displayFieldStatus(b,{isValid:false});
hm.formValidator.displayFieldStatus(g,{isValid:false,errorMessage:g.data("login-failed-text")});
a.setAttribute("disabled",false);
a.removeAttribute("disabled");
hm.Tracking.link({event_type:"LOGIN_ERROR",event_id:"Login failed",event_category:"Login",event_label:"Login failed"})
}}};
e.onerror=function(){hm.formValidator.displayFieldStatus(b,{isValid:false});
hm.formValidator.displayFieldStatus(g,{isValid:false,errorMessage:g.data("login-failed-text")});
a.setAttribute("disabled",false);
a.removeAttribute("disabled");
hm.Tracking.link({event_type:"LOGIN_ERROR",event_id:"Login failed",event_category:"Login",event_label:"Login failed"})
